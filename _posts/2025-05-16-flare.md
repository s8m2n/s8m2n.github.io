...
이렇게 얻은 local Point Token은 다시 DPT-based Transformer[^2]에 입력되어 Spatial Upsampling을 거쳐 출력값으로 Dense 한 point map 
...
* Adam Optimizer를 사용하고, learning Rate은 $1x10^(-4)$에서 시작해서 점진적으로 $1x10^(-5)$까지 줄인다.
* 64NVIDIA A800 GPU를 사용해 학습하고 입력 이미지의 해상도는 512*384, 200 epoch 동안 진행한다. 학습에 걸리는 시간은 대략 14일이 걸린다.

모델의 학습은 8개 시점의 이미지로 했지만 적게는 2Views, 많게는 25View까지의 입력에도 잘 대응하는 것을 확인했다.
