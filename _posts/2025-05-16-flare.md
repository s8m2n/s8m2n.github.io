...
이미지 토큰  $\mathcal{I} = \mathbf{I_i}_{i=1}^N$ 을 앞서 구한 coarse 한 pose Estimate $\mathcal{P_c} = \left\{ \mathbf{p}_i^{\text{coarse}}\right\}_{i=1}^N $ 와 Concatenate 하고, 추가적인 학습 가능한 Pose Token $\mathcal{Q_f}$ 도 함께 Concatenate 하여 또 다른 Transformer 구조 $F_l(\cdot)$ 에 입력한다. 
