...
> 그런데 만약 은닉층 노드들과 연결된 Weight 값이 모두 동일하다면 어떤 일이 생길까?? 이럴 경우, 노드들은 완전히 동일한 역할을 하게 되고 결국 신경망의 다양성과 학습 능력을 제한한다. 즉 각 노드가 가진 Weight 세트들은 해당 노드의 특정한 역할을 결정한다. 
{: .prompt-tip}
...
> 신경망에서는 활성화 함수로 비선형 함수를 사용해야 한다. 만약 선형함수를 사용한다고 하면 층을 아무리 깊게 쌓아도 결국 선형함수가 되기 때문에 층을 깊게 쌓는 의미가 없어진다. 즉 은닉층이 제 역할을 하지 못한다.
{: .prompt-info}
...
인공신경망을 한 마디로 표현하면 **함수**라고 할 수 있다. 값을 입력하면 그에 해당하는 값을 출력한다. 즉 인공신경망은 weight를 곱하고, bias를 더하고, 활성화 함수를 통과시키는 작은 구성 요소들이 연결된 하나의 복잡한 함수이다.\
따라서 AI를 학습한다는 것은 입력에 해당하는 출력의 예시를 반복적으로 보여주면서 입력과 출력을 연결하는 최적의 함수를 찾아가는 과정이다. 즉 신경망이 가지는 weight와 bias를 알아내는 것이 학습 목표이다. 
