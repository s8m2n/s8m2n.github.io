...
* $w_1, w_2$는 각 신호의 가중치를 나타내는 매개변수로 각 신호의 영향력을 제어한다. 즉 각 자극의 중요도를 결정하는 파라미터이다. 
* $b$는 Bias를 나타내는 매개변수로 뉴런이 얼마나 쉽게 활성화되는지 그 기준을 설정한다. 즉 신경의 민감도를 결정하는 파라미터이다.

> 인공신경망은 노드와 엣지로 이루어져있고 입력된 '자극'의 값에 weight를 곱하고 Bias를 더한 다음 활성화 함수를 통과시킨다. 이때 Weight와 Bias를 적절히 조정하여 인공 신경망이 다양한 상황에 올바르게 대응하도록 만들 수 있다. 하지만 실제로 수억개에 달하는 아 파라미터들을 일일이 구하기란 불가능하기 때문에 SGD와 같은 알고리즘들을 통해 기계가 스스로 적절한 Bias와 Weight를 찾도록 하고 이를 **모델을 학습**하는 과정이라고 한다.
{: .prompt-info}
 
위 그림을 Bias 매개변수를 포함해서 다시 그리면 아래와 같다.
...
> 그런데 만약 은닉층 노드들과 연결된 Weight 값이 모두 동일하다면 어떤 일이 생길까?? 이럴 경우, 노드들은 완전히 동일한 역할을 하게 되고 결국 신경망의 다양성과 학습 능력을 제한한다. **각 노드가 가진 Weight 세트들은 해당 노드의 특정한 역할을 결정한다.**
{: .prompt-tip}
...
> 신경망에서는 활성화 함수로 비선형 함수를 사용해야 한다. 선형함수의 합성은 선형함수이다. 따라서 만약 선형함수를 사용한다고 하면 층을 아무리 깊게 쌓아도 결국 다시 선형함수가 되기 때문에 층을 깊게 쌓는 의미가 없어지고 은닉층이 제 역할을 하지 못한다.
{: .prompt-info}

그런데 선형 Activation Function이 아예 필요없는 것은 아니다. 일반적인 MLP를 설계할때, 은닉층은 비선형함수 활성화함수를 사용하고 마지막 층에서는 주로 선형 활성화 함수를 사용한다. 비선형함수는 모델의 복잡도를 올려주지만 정보의 손실이 생길수 있다. 반면 선형함수는 입력을 그대로 출력하기 때문에 복잡도를 증가해주지는 않지만 정보의 손실을 막을 수 있다는 특징이 있다. 

특히 **MobileNetV2** 모델에서는 선형 활성화함수를 모델 중간에 사용한다. 이는 노드 수가 줄어드는 층에서는 선형함수를 사용해서 비선형성으로 인한 복잡도는 포기하되 정보의 손실을 막고, 노드 수가 늘어나는 층에서는 비선형 활성화 함수를 사용해서 복잡도를 올린다. 즉 정보의 손실을 최대한 막으면서 충분한 비선형성을 부여하고자 하는 목적이다. **따라서 딥러닝 모델을 설계할때는 정보의 손실과 모델의 복잡도 간의 Tradeoff를 적절히 고려하여 균형잡힌 모델을 만들수 있다.**
...
인공신경망을 한 마디로 표현하면 **함수**라고 할 수 있다. 값을 입력하면 그에 해당하는 값을 출력한다. 인공신경망은 weight를 곱하고, bias를 더하고, 활성화 함수를 통과시키는 작은 구성 요소들이 연결된 하나의 복잡한 함수이다.\
따라서 AI를 학습한다는 것은 입력에 해당하는 출력의 예시를 반복적으로 보여주면서 입력과 출력을 연결하는 최적의 함수를 찾아가는 과정이다. 최종적으로 신경망이 가지는 weight와 bias를 알아내는 것이 학습 목표가 다. 
...
> 표기법은 상황에 따라 다르게 표현될 수 있다. 이번 장에서는 다음과 같은 표기법을 사용한다.
...
![fig3-21](/assets/img/neural_network/fig3-21.png){: width="300" height="400"}
...
![fig3-22](/assets/img/neural_network/fig3-22.png){: width="300" height="400"}
