...
### 왜 계산 그래프를 이용하는가??
오차역전파법을 위해서 왜 계산 그래프를 사용할까?? 계산 그래프의 중요한 특징은 **국소적 계산**이 가능하다는 점이다. 계산 전체에서 어떤 일이 일어나든 자신과 관련된 정보만을 얻을 수 있다, 다시 말해 전체 Loss에 대한 Local Gradient를 구할수 있다. 

![fig5-7](/assets/img/back_propagation/fig5-7.png){: w="400", h="300"}

다시 한번 우리의 목적을 상기해보자. 우리의 목적은 Loss Function을 최적화하기 위한 Parameter의 조합을 알아내는 것이다. 각 변수들이 Loss 변화에 얼마나 큰 영향을 미치는지 알 수 있다면 변수들의 변화량을 통해 최종 Loss를 최적화 할 수 있을 것이다. 
...
역전파는 **연쇄법칙**에 따라 최종 신호에 각 노드의 국소적 미분을 곱하여 전체 Loss에 대한 각 변수의 미분을 효율적으로 계산할 수 있도록 하는 알고리즘이다. 위 예의 역전파 과정을 그림으로 표현하면 아래와 같다. 
...
덧셈노드의 연산에서는 순전파의 입력신호가 필요하지 않았지만 **곱셈노드의 연산을 위해서는 순전파의 입력신호가 필요하다.** 따라서 곱셈노드를 구현할때는 순전파의 입력신호를 변수에 따로 저장해두어야한다. 
...
사과와 오렌지 예시를 직접 오차역전파법을 사용해서 구현해보자.
...
#### Affine/Softmax 계층
마찬가지로 Weight를 곱하고 Bias를 더하는 Affine 변환도 오차역전파법을 이용하여 구현할 수 있다. 그 과정을 따라해보며 구현해보자. 
